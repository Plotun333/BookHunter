...
import unidecode
import os
tts = gTTS(text="The hunt has started", lang='en')
tts2 = gTTS(text="The hunt has ended", lang='en')
tts.save("H1S.mp3")
tts2.save("H1E.mp3")
# to start the file from python
os.system("start H1S.mp3")
...
def checkforkey(found,website,allfound):
    ...
    keywords = ['Sima','Ztraceny kral', 'Sutnar', "Teige", "Drtikol", "Devětsil", "Friml", "Fulla", "Halabala", "Sudek",
                    ...
                    'AFK Bohemians', 'Aforismy Smazik Rudolf','akcni tvorba', 'akrobat', 'aktuality budovani',
                    ...
                    'CEKANI NA GODOTA', 'Cellularni mysleni', 'celokozena', 'cerminova', 'Cerne ruze',
                    ...
                    'Saga Gunnlaugovi', 'SALUT AU MONDE', 'samizdat', 'SARAUER ALOIS', 'sbohem armado', 
                    ...
                    'Signal 1929', 'Signal Casopis', 'Simotova', 'Situace 89', 'Skleneny havelok', 'Sklepnik',
                    ...
                    'Tragedie ruska', 'Tramp Eso', 'TRAMPOTY 1932', 'Tribuna','trn 1968', 'Trojan Zvon zeme',
    ...
    return Hunted
        ...
        # http://www.samota.cz/katonline/nov/nov.html
...
webpages = ["http://antikvariat-fryc.cz/", "http://antikvariat-malyctenar.cz/", "http://antikvariatostrov.cz/?mn_offs=21",
            ...
            'http://www.spalena53.cz/knihy/', 'http://www.valentinska.cz/home',
...
for web in webpages:
    ...
    if __name__ == '__main__':
        ...
        Hunted = checkforkey(found, web, allbooks)
        if len(Hunted) > 0:
            print('\n')
            for element in Hunted:
                print(' '+str(booknum)+' ) '+str(element))
                booknum = booknum + 1
                print('-----------------------------------------------------------------------------------------------------')
                allbooks.append(element)
                print('\n')
...
for book in allbooks:
    b = book.replace('\n', "")
    b = b.replace(" ", "")
    if unidecode.unidecode(str(b[12])+str(b[13])+str(b[14]))+str(allbooks.index(str(book))) not in open('hunter.txt','r').read():
            print(str(index)+'] '+str(book)+' '+str(allbooks.index(book)))
            index = index+1
            print('-----------------------------------------------------------------------------------------------------------')
            print('\n')
            nobooks = True
...
for book in allbooks:
    b = book.replace('\n', "")
    b = b.replace(" ", "")
    add.write(unidecode.unidecode(str(b[12])+str(b[13])+str(b[14]))+str(allbooks.index(str(book))))
    add.write('\n')          
